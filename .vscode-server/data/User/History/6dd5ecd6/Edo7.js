...
function updateRecentBoards(boards, enteredPin) {
  for (const board of boards) {
    ...
    const link = boardDiv.find(".link");
    // boardDiv.find(".link").attr("href", "/board/" + board.boardId);
    link.click(() => {
      currentBoardId = board.boardId;
      const requiresPin = board.pin !== undefined && board.pin !== null;

      if (requiresPin) {
        // Prompt for PIN
        const enteredPin = prompt("Enter the board PIN:");
        ...
        if (enteredPin === board.pin) {
          // If PIN matches, proceed to the board
          window.location.href = "/board/" + board.boardId;
        } else {
          // If PIN doesn't match, show an error (you can customize this)
          alert("Invalid PIN. Please try again.");
        }
      } else {
        // If no PIN is required, directly proceed to the board
        window.location.href = "/board/" + board.boardId;
      }
    });
...
$("#create-board-button").click(() => {
  const enteredPin = prompt("Enter the PIN for the new board:");
  ...
  const createConfirmation = confirm("Create a new board?");
  if (createConfirmation) {
    // Emit an event to create a new board with the entered PIN
    socket.emit("createBoard", { pin: enteredPin }, (data) => {
...
(function () {
  ...
  btn.click(() => {
    ...
    // Validate the entered PIN (you may need more robust validation)
    if (!enteredPin || isNaN(enteredPin) || enteredPin.length !== 4) {
      alert("Invalid PIN. Please enter a valid 4-digit PIN.");
      return;
    }
