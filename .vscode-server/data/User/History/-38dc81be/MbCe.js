...
(function () {
  ...
  function createBoard() {
    const pin = prompt("Enter the PIN for the new board:");
    if (!pin || isNaN(pin)) {
      toastr.error("Invalid PIN. Please enter a valid 4-digit PIN.", "Error");
      return;
    }

    socket.emit("createBoard", { pin }, (data) => {
  ...
  function joinBoard() {
    ...
    const pin = prompt("Enter the PIN for the board:");
    if (!pin || isNaN(pin)) {
      toastr.error("Invalid PIN. Please enter a valid 4-digit PIN.", "Error");
      return;
    }

    socket.emit("joinBoard", { boardId, pin }, (data) => {
      const { status, lineHist, noteList } = data;
      if (status === "OK") {
        // Board joined successfully, handle the data as needed
        // ...
      } else {
        toastr.error("Invalid PIN or Board ID. Please check your input.", "Error");
      }
    });
  }
  ...
  function onNoteCreate() {
    ...
    socket.emit("createNote", { id, x, y, w, h, msg, color });
  }
