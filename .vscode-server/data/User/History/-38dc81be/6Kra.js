...
(function () {
  ...
  socket.emit("load", null, (data) => {
    ...
    if (!data || typeof data !== 'object') {
      // Handle the case where the received data is not as expected
      console.error('Invalid data received during board load:', data);
      return;
    }
    ...
    if (status === "NOT_FOUND" || !pin || !promptPin(pin)) {
      $.confirm({
        // Display a prompt for the PIN
        theme: "supervan",
        icon: "fas fa-key",
        title: "Enter PIN",
        content: `
        ...
        buttons: {
          ok: function () {
            ...
            if (enteredPin === pin) {
              loadBoard(lineHist, noteList);
            } else {
              // Display an error message and reload the page or handle accordingly
              window.location.href = "/";
            }
          },
          cancel: function () {
